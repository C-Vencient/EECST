...
class MAPPO_SMAC:
    ...
    def choose_action(self, obs_n, avail_a_n, evaluate):
        # 这个函数的作用为，首先将各个智能体的观测obs_n加入到actor_inputs列表中
        # 再遍历列表中的各个元素，将其合并成一个全局信息的观测，即状态（actor_inputs）
        # 将这个状态输入，获得动作的概率分布prob
        # 如果是评估状态下，则获取概率最高的动作
        # 反之从这个概率分布中抽样动作
        with torch.no_grad():
            actor_inputs = []
    ...
    def get_value(self, s, obs_n):
        # 这个函数的作用是：首先，将所有智能体观测构成的状态s升维，具体是(state_dim,)-->(N,state_dim)
        # 可以这样做是由于每个对于智能体而言，其全局观测（即状态）是一样的
        # 之后，将这个状态s加入到critic_inputs中作为critic网络的输入
        with torch.no_grad():
            critic_inputs = []
    ...
    def train(self, replay_buffer, total_steps):
        ...
        with torch.no_grad():  # adv and v_target have no gradient
            ...
            for t in reversed(range(max_episode_len)):
                # 使用 deltas[:, t] 时，实际上是在选择 deltas 张量中第二维（时间步维度）的第 t 个切片
                # 这将给出一个形状为 (batch_size, N) 的张量。
